...
namespace IBM.Watsson.Examples
{
    public class CommandVoice : MonoBehaviour
    {
        ...
        public Text password,say;
        private AudioSource _as;
        public AudioClip success,fail;
        public Image background;
        private bool wordmatch= false;
        ...
        void Start()
        {
            ...
            indexWords = Random.Range(0,words.Length-1);
            password.text = words[indexWords];
        ...
        private void OnRecognize(SpeechRecognitionEvent result)
        {
            if (result != null && result.results.Length > 0)
            {
                foreach (var res in result.results)
                {
                    ...
                    {
                        string resultado = alt.transcript;
                        // string resultado = alt.transcript.Split(' ')[0];
                        string []palabras = resultado.Split(' ');
                        
                        // say.text = resultado;
                        say.text = palabras[0];

                        if(palabras[0].Equals(words[indexWords]))
                        {
                            if(!wordmatch)
                            {
